...
class postHandler {
    ...
    public function addDespesa($opcao, $data, $valor, $descricao) {
        $idUsuario = $_SESSION['login'];
        $sql = $this->pdo->query("INSERT INTO registros (descricao, data_registro, valor_registro, tipo_registro, identificador, data_e_hora) values ('$descricao', '$data', '$valor', '$opcao', $idUsuario, now())");
        if ($sql->rowCount() > 0) {
            return true;
        }else {
            return false;
